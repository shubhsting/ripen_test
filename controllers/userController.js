...
async function sendOTP(req,res) {
    try{
        ...
        if(!user || user.is_verified || phone.length !== 10) return res.status(400).send("Already verified user or Invalid attempt");
        if(user.ts && parseInt(user.ts) > moment().subtract(25,'seconds').valueOf()) return res.status(400).send("Kindly retry after 20 seconds");
